...
export default function EditBill() {
  ...
  const targetBillId = params.id || params.billId; 
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const bill = await db.bills.get(parseInt(targetBillId));
        if (bill) {
          setDate(bill.date);
          setSelectedShopId(bill.shopId.toString());
          
          if (bill.receivedAmount >= 0) {
            setIsTodayPaymentEdited(true);
            setCustomTodayPayment(bill.receivedAmount.toString());
          }
          
          setPastDuePayment(bill.pastDueReceived ? bill.pastDueReceived.toString() : '');
          setRemarks(bill.remarks || '');
          
          const bItems = await db.billItems.where({ billId: parseInt(targetBillId) }).toArray();
          const qts = {};
          bItems.forEach(bi => {
             qts[bi.itemId] = bi.quantity;
          });
          setQuantities(qts);
        } else {
          navigate('/home'); 
    ...
    loadData();
  }, [targetBillId, navigate]);

  useEffect(() => {
    const fetchPreviousDue = async () => {
      if (selectedShopId) {
        try {
          const shopBills = await db.bills.filter(bill => String(bill.shopId) === String(selectedShopId) && bill.id !== parseInt(targetBillId)).toArray();
          let totalDue = 0;
  ...
  const handleUpdateBill = async (e) => {
    ...
    try {
      await db.bills.update(parseInt(targetBillId), {
  ...
  return (
    <div className={`h-dvh ${theme.colors.background} flex flex-col relative overflow-hidden transition-colors duration-300`}>
      ...
      <PageHeader title={currentStep === 1 ? t.editBillTitle || "Edit Bill" : t.billSummaryTitle} onBack={handleBack} />
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-4 pb-24 hide-scrollbar">
        {currentStep === 1 && (
          <div className="space-y-4">
            <div className="flex gap-4">
              ...
              <div className="w-1/2">
                 <FormSelect label={t.shopLabel} value={selectedShopId} onChange={(e) => setSelectedShopId(e.target.value)} options={shopOptions} disabled icon={Store} required />
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-end mb-3">
                <h3 className={`font-bold text-[#14348c] dark:text-blue-400 text-[16px] flex items-center gap-2`}><ShoppingCart size={20} /> {t.selectItemsLabel}</h3>
                <span className="text-xs font-bold text-[#14348c] dark:text-blue-300 bg-blue-100 dark:bg-blue-900/50 px-2 py-1 rounded-lg">
                  {t.selectedCountLabel} {activeBillItems.length}
                </span>
              </div>

              <div className="relative mb-4">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Search size={18} className={theme.colors.mutedText} /></div>
                <input 
        ...
        {currentStep === 2 && (
          <form onSubmit={handleUpdateBill} className="space-y-6">
            <div>
              <h3 className="font-bold text-[#14348c] dark:text-blue-400 mb-3 text-[16px] flex items-center gap-2"><CheckCircle2 size={20} className="text-green-600" /> {t.selectedItemsLabel}</h3>
              <div className="space-y-3">
      ...
      <div className={`flex-none ${theme.colors.navBg} border-t ${theme.colors.navBorder} px-4 py-3 pb-safe shadow-[0_-4px_10px_rgba(0,0,0,0.03)] z-50`}>
        {currentStep === 1 ? (
          ...
          <PrimaryButton onClick={handleUpdateBill} icon={Save} className="bg-green-600! hover:bg-green-700! dark:bg-green-700! dark:hover:bg-green-800!">
            {t.saveChanges}
          </PrimaryButton>
        )}
      </div>
